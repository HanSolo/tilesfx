...
public class ClusterMonitorTileSkin extends TileSkin {
    ...
    @Override protected void initGraphics() {
        ...
        updateHandler    = new ChartDataEventListener() {
            @Override public void onChartDataEvent(final ChartDataEvent e) {
                switch (e.getType()) {
                    case UPDATE:
                        ClusterMonitorTileSkin.this.updateChart();
                        break;
                    case FINISHED:
                        ClusterMonitorTileSkin.this.updateChart();
                        break;
        ...
        paneSizeListener = new InvalidationListener() {
            @Override public void invalidated(final Observable e) {
                ClusterMonitorTileSkin.this.updateChart();
            }
        };
        ...
        if (null != svgPath) {
            svgPathPressedHandler = new EventHandler<MouseEvent>() {
                @Override public void handle(final MouseEvent e) {
                    tile.fireTileEvent(SVG_PRESSED_EVENT);
                }
            };
    ...
    @Override protected void registerListeners() {
        ...
        tile.getChartData().addListener(new WeakListChangeListener<>(new ListChangeListener<ChartData>() {
            @Override public void onChanged(final Change<? extends ChartData> change) {
                while (change.next()) {
                    if (change.wasAdded()) {
                ...
                for (Entry<ChartData, ChartItem> entry : dataItemMap.entrySet()) {
                    chartPane.getChildren().add(entry.getValue());
                }
                ClusterMonitorTileSkin.this.updateChart();
            }
        }));
    ...
    @Override public void dispose() {
        ...
        for (BarChartItem item : tile.getBarChartItems()) {
    ...
    @Override protected void resize() {
        ...
        for (ChartItem item : dataItemMap.values()) {
    ...
    private class ChartItem extends Region {
        ...
        public ChartItem(final ChartData CHART_DATA, final CtxBounds CONTENT_BOUNDS, final String FORMAT_STRING) {
            ...
            compressed        = false;
            chartDataListener = new ChartDataEventListener() {
                @Override public void onChartDataEvent(final ChartDataEvent e) {
                    switch (e.getType()) {
                        case UPDATE:
                            ChartItem.this.update();
                            break;
                        case FINISHED:
                            ChartItem.this.update();
                            break;
